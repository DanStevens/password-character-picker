...
async function loadPreferencesAsync() {
    ...
    throw new Error("Storage with key 'preferences' missing.");
}

async function main() {
    try {
        preferences = await loadPreferencesAsync();
        console.log("Preferences received from storage", preferences);
    } catch (error) {
        console.log("Error retrieving preferences", error, "Using default preferences", preferences);
    }
    ...
    document.body.classList.remove('backgroundIcon');
